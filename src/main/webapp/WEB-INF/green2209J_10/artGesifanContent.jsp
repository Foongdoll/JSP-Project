...
<body style="height: 1400px">
	...
	<h2 style="margin-top: 70px">게 시 글</h2>
		<table class="table table-borderless" style="background-color: #ccc;">
			<tr>
				<td colspan="4">
					<h3><button type="button" onclick="location.href='${ctp}/artGesifan.ge?pag=${pag}&pageSize=${pageSize}'" class="badge badge-pill badge-secondary" >자유게시판 가기</button></h3>
				</td>
			</tr>
			<tr>
				<td colspan="2">
			...
			<tr>
				<td colspan="2">
				<font color="4077E5">글쓴이</font> : 
				<c:if test="${sLevel == 0 }"><i class="fa-sharp fa-solid fa-star"></i></c:if> 
				<c:if test="${sLevel == 1 }"><i class="fa-sharp fa-solid fa-user"></i></c:if> 
				<c:if test="${sLevel == 2 }"><i class="fa-sharp fa-solid fa-user"></i></c:if> 
				${vo.nickName} <font color="4077E5">추천</font> : ${vo.good } <font color="4077E5">조회수</font> : ${vo.readNum} ${fn:substring(vo.wDate,2,19)}
				</td>
			</tr>
			<tr>
				...
				<td colspan="4"><c:if test="${vo.fSName == null}">사진없음</c:if>
					<c:if test="${vo.fSName != null}">
						<h3><span class="badge badge-fill badge-secondary">첨부한 이미지</span></h3>
							<img src="${ctp}/data/artGesifan/${vo.fName}" width="70%">
					</c:if>		
				</td>
			</tr>
			<tr>
					...
					<table class="table">
						<c:forEach var="Comvo" items="${Comvos}" varStatus="st">
							<tr class="text-center">
								...
								<th style="width: 20%">
									<c:if test="${Comvo.day_diff > 0 && Comvo.hour_diff > 24}">${fn:substring(Comvo.wDate,0,11)}</c:if>
    		       		<c:if test="${Comvo.day_diff > 0 && Comvo.hour_diff <= 24}">${fn:substring(Comvo.wDate,0,19)}</c:if>
		           		<c:if test="${Comvo.day_diff < 1}">${fn:substring(Comvo.wDate,11,19)}</c:if>
									<c:if test="${Comvo.nickName == sNickName || sLevel == 0}">
